...
export function generateStaticParams() {
  return staticGenericItemParams.visa;
}

export async function generateMetadata({ params }: PageProps) {
  const { item } = await params;
  const slug = decodeURIComponent(item);
  const detail = getServiceItemDetail("visa", slug);

  return { title: `${detail.option?.name ?? titleFromSlug(slug)} | An Khai Travel` };
}

export default async function VisaItemPage({ params }: PageProps) {
  const { item } = await params;
  const slug = decodeURIComponent(item);
  const detail = getServiceItemDetail("visa", slug);

  return <GenericServiceItemDetailView serviceId="visa" itemSlug={slug} option={detail.option} serviceLabel="Visa" />;
}
